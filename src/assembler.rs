...
fn test_literals() {
    ...
    assert_eq!(compile_and_run("255"), 255);
}

#[test]
fn test_addition() {
    assert_eq!(compile_and_run("5+3"), 8);
    assert_eq!(compile_and_run("1+2+3"), 6);
}

#[test]
fn test_subtraction() {
    assert_eq!(compile_and_run("10-3"), 7);
    assert_eq!(compile_and_run("5-2-1"), 2);
}

#[test]
fn test_multiplication() {
    assert_eq!(compile_and_run("2*3"), 6);
    assert_eq!(compile_and_run("5+6*7"), 47);  // Your example!
}

#[test]
fn test_division() {
    assert_eq!(compile_and_run("8/2"), 4);
    assert_eq!(compile_and_run("9/2"), 4);  // Integer division
}
